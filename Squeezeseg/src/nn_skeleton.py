...
class ModelSkeleton:
  ...
  def _add_output_graph(self):
    ...
    with tf.compat.v1.variable_scope('interpret_output') as scope:
      self.prob = tf.multiply(
          ...
          name='pred_prob')
      self.pred_cls = tf.argmax(input=self.prob, axis=3, name='pred_cls')
  ...
  def _add_loss_graph(self):
    ...
    with tf.compat.v1.variable_scope('cls_loss') as scope:
      self.cls_loss = tf.identity(
          ...
          )/tf.reduce_sum(input_tensor=self.lidar_mask)*mc.CLS_LOSS_COEF,
          name='cls_loss'
      )
  ...
  def _conv_bn_layer(
    ...
    with tf.compat.v1.variable_scope(conv_param_name) as scope:
      ...
      conv = tf.nn.conv2d(
          input=inputs, filters=kernel, strides=[1, 1, stride, 1], padding=padding,
          name='convolution')
      if conv_with_bias:
    ...
    with tf.compat.v1.variable_scope(layer_name) as scope:
      ...
      for it in range(num_iterations):
        ...
        ang_output = tf.nn.conv2d(
            input=ang_output, filters=angular_compat_kernel, strides=[1, 1, 1, 1],
        ...
        bi_output = tf.nn.conv2d(
            input=bi_output, filters=bi_compat_kernel, strides=[1, 1, 1, 1], padding='SAME',
  ...
  def _locally_connected_layer(
    ...
    with tf.compat.v1.variable_scope(layer_name) as scope:
      # message passing
      ang_output = tf.nn.conv2d(
          input=inputs, filters=angular_filters, strides=[1, 1, 1, 1], padding=padding,
          name='angular_filtered_term'
      )

      bi_ang_output = tf.nn.conv2d(
          input=inputs, filters=bi_angular_filters, strides=[1, 1, 1, 1], padding=padding,
          name='bi_angular_filtered_term'
      )

      condensed_input = tf.reshape(
          tf.nn.conv2d(
              input=inputs*self.lidar_mask, filters=condensing_kernel, strides=[1, 1, 1, 1], padding=padding,
      ...
      bi_output = tf.multiply(
          tf.reduce_sum(input_tensor=condensed_input*bilateral_filters, axis=3),
          self.lidar_mask,
  ...
  def _bilateral_filter_layer(
    ...
    with tf.compat.v1.variable_scope(layer_name) as scope:
      ...
      condensed_input = tf.nn.conv2d(
          input=inputs, filters=condensing_kernel, strides=[1, 1, stride, 1], padding=padding,
  ...
  def _activation_summary(self, x, layer_name):
    ...
    with tf.compat.v1.variable_scope('activation_summary') as scope:
      ...
      tf.compat.v1.summary.scalar(layer_name+'/average', tf.reduce_mean(input_tensor=x))
      tf.compat.v1.summary.scalar(layer_name+'/max', tf.reduce_max(input_tensor=x))
      tf.compat.v1.summary.scalar(layer_name+'/min', tf.reduce_min(input_tensor=x))
